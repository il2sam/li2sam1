...
function renderQuestion() {
  ...
  choices.forEach((choice) => { const button = document.createElement("button"); button.type = "button"; button.className = "choice-button"; button.textContent = choice; button.addEventListener("click", () => choose(choice, button)); choicesBox.append(button); });
...
function choose(choice, button) {
  ...
  document.querySelectorAll(".choice-button").forEach((item) => { item.disabled = true; if (item.textContent === question.answer) item.classList.add("correct"); });
